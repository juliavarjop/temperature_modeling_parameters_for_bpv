function RMSdifference = calculateRMSdifference_MPV_v1(fitVariables,conditions)
...
a_coeff=fitVariables(1);
b_coeff=fitVariables(2);

disp(['Trying a = ', num2str(fitVariables(1)), ', b = ', num2str(fitVariables(2))])
...
for idxConditions = 1:m
    ...
    while (Tchange > TendCriterion && countT < countTlimit)
        try
            ...
            EffRefAtT = effRefFront*(1+(n*consts.kB*EffRefT)...
                /(VocRefG*consts.q)*log((I_Front_R(idxConditions))/(EffRefG)))*(1+EffShiftVsTSi*(Tcell-EffRefT)/100);
            ...
            % % Build Comsol model VBPV
            % [~,TcellAve,TcellAveTop,TcellAveBack,TmodAve,TcellMax,~,~,~,~,~,~,...
            %     CutLineCells,CutLineTop,CutLineBack,CutLineCellsX,CutLineTopX,CutLineBackX] = ...
            %     VBPV_SipanelTcomsolModel(Tamb(idxConditions),v_wind(idxConditions),Qcell,pathFolder,Tcell,a_coeff,b_coeff);

            % Build Comsol model MPV and CBPV
            [~,TcellAve,TcellAveTop,TcellAveBack,TmodAve,TcellMax,~,~,~,~,~,~,...
                CutLineCells,CutLineTop,CutLineBack,CutLineCellsX,CutLineTopX,CutLineBackX] = ...
                MPV_CBPV_SipanelTcomsolModel(Tamb(idxConditions),v_wind(idxConditions),Qcell,pathFolder,Tcell,a_coeff,b_coeff);
...
% fitY = fitVariables(1)*conditions.v_wind+fitVariables(2); 
...
for indm = 1:m 
    ...
    TmaxBack(indDataPoint,2) = Tamb(indm);
    TmaxBack(indDataPoint,3) = v_wind(indm);
    TmaxBack(indDataPoint,4)=max([listMAX{:}])-273.15;
    ...
    TminBack(indDataPoint,2) = Tamb(indm);
    TminBack(indDataPoint,3) = v_wind(indm);
    TminBack(indDataPoint,4) = min([listMIN{:}])-273.15;
    % 
    ...
    TaveCells(indDataPoint,2) = Tamb(indm);
    TaveCells(indDataPoint,3) = v_wind(indm);
    TaveCells(indDataPoint,4) = TaveCells_vs_Si(indm,ind)-273.15;
    ...
    TaveBack(indDataPoint,2) = Tamb(indm);
    TaveBack(indDataPoint,3) = v_wind(indm);
    TaveBack(indDataPoint,4) = TcellAveBack_vs_Si(indm,ind)-273.15;
    
    indDataPoint = indDataPoint+1;

end

% assignin('base','TminBack',TminBack)

%%% Calculate RMSE %%%

TmodM=conditions.T(:,1);
TmodS=TmaxBack(:,4);
