function RMSdifference = calculateRMSdifference_VBPV_v1(fitVariables,conditions)
...
I_Front_R=conditions.POAEast;
I_Rear_R=conditions.POAWest;
...
for idxConditions = 1:m
    ...
    while (Tchange > TendCriterion && countT < countTlimit)
        try
            ...
            %%% Qcell Back
            % Electrical efficiency
            EffRefAtT = effRefBack*(1+(n*consts.kB*EffRefT)/(VocRefG*consts.q)...
                *log((I_Rear_R(idxConditions))/(EffRefG)))*(1+EffShiftVsTSi*(Tcell-EffRefT)/100);
            effEref = EffRefAtT/totAbsCoeff;
